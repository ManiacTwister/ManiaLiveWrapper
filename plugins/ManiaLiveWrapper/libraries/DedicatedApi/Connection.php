 ...
 * @date        $Date: 2013-05-17 11:02:37 +0200 (ven., 17 mai 2013) $:
 */
...
/* Multiquery */
	...
	function multiquery()
	{
		...
		if(count($this->calls))
		{
			$result = $aseco->client->query('system.multicall', $this->calls);
	...
	function multiqueryIgnoreResult()
	{
		...
		if(count($this->calls))
		{
			$aseco->client->queryIgnoreResult('system.multicall', $this->calls);
			$this->calls = array();  // reset for next calls
		}
	}
/* Multiquery end */
	...
	function executeCallbacks()
	{
		global $aseco;
		$aseco->client->readCB();
		$calls = $aseco->client->getCBResponses();
		return $calls;
	}
	...
	protected function execute($methodName, $params = array(), $multicall = false)
	{
		...
		if($multicall)
		{
			$this->addCall($methodName, $params);
		}
		else
		{
			array_unshift($params, $methodName);
			call_user_func_array(array($aseco->client, 'query'), $params);
			return $aseco->client->getResponse();
		}
	}
